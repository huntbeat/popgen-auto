...
Example command:
python3 hmm_cnn.py -d /scratch/saralab/third/strength0.msms -n 8 -p /scratch/saralab/first/million.txt -i 1 -b 16
"""
...
def main():
    ...
    with open(opts.dif_fasta_filename, 'r') as inputFasta:
        ...
        num_times = int(input_string.split(" ")[4])
        print("The length of the sequence is %d." % length)
        print("The number of individuals  is %d." % num_indivs)
        print("The number of iterations   is %d." % num_times)
        input_param = "_".join(input_string.split(" ")[:-3])
        output_filename = '/scratch/saralab/third/TMRCA' + input_filename.replace('.msms','.txt')
        print('Output file : ' + output_filename)
        output_TMRCA = open(output_filename, 'w')
