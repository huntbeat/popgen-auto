...
def chrom2msms(vcf_filename, chrom, sample_size, start, length, sample_list=None):

    header_written = False
    msms_filename = '/scratch/saralab/first/chrom2_MXL_long.msms'
    ...
    while start < end:
        ...
        assert(len(reformatted_hap_list) == 20)
        assert(len(reformatted_hap_list[0]) == len(reformatted_seg_site_idx_list))
...
def main():
    ...
    chrom2msms(vcf_filename='/scratch/hlee6/vcf/ALL.chr2.vcf.gz', chrom=2, sample_size=20, start=0, length=5000000, sample_list=MXL_pop)
    #chrom2msms(vcf_filename='/scratch/hlee6/vcf/ALL.chr2.vcf.gz', chrom=2, sample_size=20, start=0, length=100000, sample_list=MXL_pop)
