...
def main():
    ...
    try:
        ...
        training_generator = DataGenerator(data_file, partition['train'], 2)
        validation_generator = DataGenerator(data_file, partition['validation'], 2)
    ...
    # MSMS #
    eval_generator = DataGenerator(data_file, partition['test'], 2)
    '''
    n_print = 20
    n_predict = int(n_print/32)*32 + 32
    predict_generator = DataGenerator(data_file, partition['test'][:n_predict], 2)
    ...
    real_generator = DataGenerator(real_file, real_indices, 2, real_data=True)
